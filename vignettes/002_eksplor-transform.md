...
colnames(iris) # melihat isi dari row dan column
```
...
head(iris) # melihat 5 data teratas
```
...
``` r
tail(iris) # melihat 5 data terbawah
```
...
``` r
summary(iris) # melihat ringkasan statistik
```
...
``` r
class(iris$Species) # melihat class
```

    ## [1] "factor"

``` r
length(iris$Petal.Width) # melihat length dari objek
```

    ## [1] 150

``` r
levels(iris$Species) # melihat level atribut dari variabel
```
